...
class PlanningModel(TorchModuleWrapper):
    ...
    def forward(self, data
        ...
        target = data["agent"]["target"][:, 0] # [bs, future_steps, 3]
        # 打印data[current_state]的键.结果是一个纯数字tensor
        # print("target:",target.shape)
        # sys.exit(1)

        bs, A = agent_pos.shape[0:2] # bs=32, A=33
        ...
        #其它代理的嵌入表示
        other_instance_feature = x[:, 1:A] #torch.Size([32, 32, 128])   其它代理的嵌入表示,形状为 (batch_size, num_agents, embed_dim)
        ...
        # traj_anchors_np通过复制增加一维变为[bs,num_modes,80,4]
        traj_anchors = traj_anchors.unsqueeze(0).expand(bs, -1, -1, -1)
